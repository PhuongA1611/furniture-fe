...
const AddressDetail = () => {
  ...
  useEffect(() => {
    if (provinceValue && location) {
      ...
      setListDistrict(
        provinceData.districts.map((item, index) => {
          return {
            code: item.code,
            name: item.name,
          };
  ...
  const handleDelete = () => {
    try {
      const id = addressId;
      dispatch(deleteShipping(addressId))
        .unwrap()
        .then((data) => {
  ...
  const onSubmit = (params) => {
    ...
    try {
      if (addressId) {
        const id = addressId;
        dispatch(updateShipping({ id, params }))
          .unwrap()
          .then((data) => {
            message.success(data.message);
            navigate(-1);
        ...
        dispatch(createShipping({ userId, params }))
          .unwrap()
          .then((data) => {
            message.success(data.message);
            navigate(-1);

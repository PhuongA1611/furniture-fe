...
export const getProvince = createAsyncThunk(
  "order/getProvince",
  async (params) => await orderApi.getProvince(params)
);

export const createOrder = createAsyncThunk(
  "order/createOrder",
  async (params) => await orderApi.createOrder(params)
);

export const getListOrder = createAsyncThunk(
  "order/getListOrder",
  async (params) => await orderApi.getListOrder(params)
);

export const getAllOrder = createAsyncThunk(
  "order/getAllOrder",
  async (params) => await orderApi.getAllOrder(params)
);

export const getOrderDetail = createAsyncThunk(
  "order/getOrderDetail",
  async (id) => await orderApi.getOrderDetail(id)
);

export const updateStatus = createAsyncThunk(
  "order/updateStatus",
  async (params) => await orderApi.changeStatus(params)
);
  ...
  extraReducers: (buider) => {
    ...
    buider.addCase(getAllOrder.fulfilled, (state, action) => {
      ...
      action.payload.data &&
        action.payload.data.map((item, index) => {
          const totalPrice =
            item.orderDetail.reduce((total, item) => total + item.price, 0) ||
            0;
    ...
    buider.addCase(getOrderDetail.fulfilled, (state, action) => {
      ...
      if (action.payload.data.ordersDetail) {
        state.totalPrice =
          action.payload.data.ordersDetail.reduce(

  ...
  return (
    <div className="my-4 mx-5 admin-order">
      ...
      {list && (
        ...
        >
          <Column title="Index" key="index" render={(value, item, index) => (pageCurrent - 1) * pageSize + index + 1} />
          <Column title="Code" key="id" dataIndex="id" render={(_, record) => '#' + record.id} />
          <Column
            title="Status"
            // dataIndex="status"
            key="status"
          ...
          />
          <Column title="" key="action" render={(_, record) => <Link to={`` + record.id}>View</Link>} />
        </Table>
      )}
